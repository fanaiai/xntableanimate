...
(function (window) {
    ...
    XNQuery.prototype = init.prototype = {
        ...
        setCss: function (dom1, ele) {
            if (!dom1 || !ele) {
                return;
            }
            var dom = dom1;
            for (let i in ele.css) {
                if (i == 'position' && ele.css[i] == 'fixed') {
                    continue;
                }
                if (ele.css[i] == 'none') {
                    dom.style[i] = ele.css[i];
                    continue;
                }
                // if (i == 'transform') {
                //     dom.style['transformOrigin'] = "center";//旋转中心
                //     dom.style[i] = this._geneTransform(ele.css[i])
                //     continue;
                // }
                // if (i == 'boxShadow') {
                //     dom.style[i] = this._geneShadow(ele.css[i])
                //     continue;
                // }
                // if (i == 'textShadow') {
                //     dom.style[i] = this._geneTextShadow(ele.css[i])
                //     continue;
                // }
                if (i == 'position') {
                    dom.style[i] = ele.css[i];
                ...
                if (i == 'backgroundImage') {
                    // if (typeof ele.css[i] == 'object') {
                    //     dom.style[i] = this._rendGradient(ele.css[i]);
                    //     continue;
                    // } else {
                    dom.style[i] = "url(" + ele.css[i] + ")";
                    continue;
                    // }
                } else {
                    if (this._isNumber(ele.css[i]) && i != 'zIndex') {
                        if (i != 'flex') {//处理flex
                            dom.style[i] = parseFloat(ele.css[i]) + "px";
                        } else {
                            dom.style[i] = ele.css[i]
                        }
                        ...
                        dom.style[i] = ele.css[i];
                        if (i == 'height') {//计算高度
                            var height = $("#div" + ele.id).height();
                            dom.style[i] = height + "px";
                        }
                    }
                    // if (isNaN(parseFloat(ele.css[i])) || i == 'zIndex' || String(ele.css[i]).lastIndexOf("%") > -1) {
                    //     dom.style[i] = ele.css[i];
                    // } else {
                    //     dom.style[i] = parseFloat(ele.css[i]) + "px";
                    // }
                ...
                // if (i == 'filter') {
                //     dom.style[i] = this.rendFilter(ele.css[i]);
                // }
            }
    ...
    XNQuery.isNumber = XNQuery.prototype.isNumber;
    XNQuery.setCss = XNQuery.prototype.setCss;
    XNQuery._isNumber = XNQuery.prototype._isNumber;
